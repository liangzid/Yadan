...
// pub mod core_toda{
    ...
    pub type Database=HashMap<Domain,i32>;
    pub type Act=(String,String,String,String);
    ...
    pub fn encode_dialogue_act(acts:&Vec<Act>)->String{
        ...
        for act in acts{
            ...
            let encoded_act:String=intent.to_owned()+&seperator+&domain+&seperator+&slot+&seperator+&value+&seperator+&big_sep;
    ...
    // transfer a string acts into a acts structure.
    pub fn decode_database(e_db:&str)->Database{
        ...
        for d in dbs{
            if d==""{
                ...
                let num:i32=info.split(" ").nth(1).unwrap_or_default().to_string().parse::<i32>().unwrap_or_default();
    ...
    impl Dialogues{
        ...
        pub fn get_neuralpipeline_inputs(&self)->Vec<(Vec<String>,Belief,Database,Vec<Act>,String)>{
            let mut results:Vec<(Vec<String>,Belief,Database,Vec<Act>,String)>=vec![];
            ...
            for dialogue in &self.dialogues{
                ...
                for item in &dialogue.items{
                    if let Some(field)=&item.get("belief"){
                        // println!("{:?}---",&item);
                        let system_item:ItemBySystem=serde_json::from_str(&item.to_string()).unwrap();
        ...
        pub fn get_neuralpipeline_string_intputs(&self,model_type:&str,split_list:&Vec<String>)->Vec<String>{
            ...
            if model_type=="soloist"{
                ...
                for perreuslt in running_results{
                    ...
                    let per_str:String=split_list[0].clone()+&encode_history(&history)
                                        +&split_list[1]+&encode_belief(&bs)+&split_list[2]
                                        +&encode_database(&db)+&split_list[3]
                                        +&response+&split_list[4];
                ...
                for perreuslt in running_results{
                    ...
                    for (i,history) in perreuslt.iter().enumerate(){
                        ...
                        if i==perreuslt.len()-1{
                            let sub_turn=split_list[2].clone()+utterance+&split_list[3]
                                                +&encode_belief(bs)+&split_list[4]+&encode_database(db)
                                                +&split_list[5]+&encode_dialogue_act(acts)+response+&split_list[6];
                            this_turn_results+=&sub_turn;
                        }
                        else{
                            let sub_turn=(*utterance).clone()+&encode_belief(bs)+&encode_database(db)
                                                    +&encode_dialogue_act(acts)+response+&split_list[1];
                            this_turn_results+=&sub_turn;
                        } 
                    }
                ...
                for perreuslt in running_results{
                    ...
                    let per_str:String=split_list[0].clone()+&encode_history(&history)
                                        +&split_list[1]+&encode_belief(&bs)+&split_list[2]
                                        +&encode_database(&db)+&split_list[3]
                                        +&encode_dialogue_act(&acts)+&split_list[4]
                                        +&response+&split_list[5];
                    results.push(per_str);
                }
            }
            results
        ...
        pub fn get_soloist_neuralpipeline_inputs(&self)->Vec<(Vec<String>,Belief,Database,String)>{
            let mut results:Vec<(Vec<String>,Belief,Database,String)>=vec![];
            // let mut history_perturn:Vec<String>;
        ...
        // in this turn.
        pub fn get_UBAR_neuralpipeline_inputs(&self)->Vec<Vec<(String,Belief,Database,Vec<Act>,String)>>{
            let mut results:Vec<Vec<(String,Belief,Database,Vec<Act>,String)>>=vec![];

            for dialogue in &self.dialogues{
                let mut history:Vec<(String,Belief,Database,Vec<Act>,String)>=vec![];
